...
class AssetsService
{
    ...
    {
        if (!empty($file)) {
            return $this->get("{$entry}/{$this->getEntry()}/css/{$file}");
        } else {
    ...
    {
        if (!empty($file)) {
            return $this->get("{$entry}/{$this->getEntry()}/js/{$file}");
        } else {
     ...
     * @inheritDoc
     */
    public function enqueueScript(string $handle, $file = '', array $deps = [], bool $footer = true): ScriptService
    {
        $file = \is_array($file)
     ...
     * @inheritDoc
     */
    public function registerScript(string $handle, $file = '', array $deps = [], bool $footer = true): ScriptService
    {
        $file = \is_array($file)
            ? $this->js($file[0], $file[1])
            : $this->js($file);
     ...
     * @inheritDoc
     */
    public function enqueueStyle(string $handle, $file = '', array $deps = [], string $media = 'all'): StyleService
    {
        $file = \is_array($file)
     ...
     * @inheritDoc
     */
    public function registerStyle(string $handle, $file = '', array $deps = [], string $media = 'all'): StyleService
    {
        $file = \is_array($file)
            ? $this->css($file[0], $file[1])
            : $this->css($file);
